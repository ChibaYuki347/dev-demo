...
│       └── sync-issues-to-ado.yml       # Topic ③ Boards sync (workflow_dispatch + guarded)
├── .vscode/
...
│   ├── main.bicep
│   └── modules/{servicebus,functionapp}.bicep
├── scripts/                             # setup-oidc, run-playwright, validate-bicep, ...
└── docs/                                # narrative for each topic + tour script
```

## Topic ↔ file map / 領域 ↔ ファイル対応

| Topic | Primary files |
|---|---|
| ① Playwright E2E | `app/frontend/playwright.config.ts`, `tests/e2e/*.spec.ts`, `.github/workflows/playwright.yml` |
| ② MCP test maintenance | `.vscode/mcp.json`, `docs/02-mcp-test-maintenance.md` |
...
## Topic ⑤ — OIDC trust chain in detail / OIDC 信頼関係詳細
...
sequenceDiagram
    ...
    GH->>Entra: azure/login@v3 exchanges JWT
